...
typedef struct Nodo{
	...
	int cantCaminos;			// Cantidad de nodos conectados
} Nodo;
...
int main() {
	...
	Nodo *nodos[] = {
		nodoA, nodoB, nodoC, nodoD, nodoE, nodoF, nodoG,
		nodoH, nodoI, nodoJ, nodoK, nodoL, nodoM, nodoN,
		nodoO, nodoP, nodoQ, nodoR, nodoS, nodoT
	};
...
Nodo* crearNodo(char id) {
	...
	nuevoNodo->cantCaminos = 0;

	return nuevoNodo;
}

void agregarConexionUnidireccional(Nodo* nodoOriginal, Nodo* nodoSalida, int peso) {
	nodoOriginal->cantCaminos++;
	int indice = nodoOriginal->cantCaminos;
...
// establecer la conexión bidireccional (grafo no dirigido)
...
TablaResultados *dijkstra(Nodo **nodos, int numNodos, int indiceInicio)
{
	...
	resultado->distancias = (int*) malloc(numNodos * sizeof(int));	//reservar memoria para arreglo de distancias
	resultado->anteriores = (int*) malloc(numNodos * sizeof(int));	//reservar memoria para el arreglo de nodos anteriores
	...
	for (int i = 0; i < numNodos; i++)
	{
		...
		visitados[i] = 0;				// por default, ninguna esta visitado
	}
	resultado->distancias[indiceInicio] = 0;

	//paso 2. encontrar el camino mas corto para todos los nodos
	for (int conteo = 0; conteo < numNodos - 1; conteo++)
	{
		...
		// si el nodo con menor distancia sigue siendo INF, los demás son inalcanzables
		if (distMinActual == INF) break;
		...
		for (int i = 0; i < actual->cantCaminos; i++)
		{
			...
			// comparar distancias al nodo destino a traves del nodo actual y la distancia previamente registrada
			if (!visitados[idxDestino] && resultado->distancias[nodoMin] + pesoAdyacente < resultado->distancias[idxDestino])
			{
